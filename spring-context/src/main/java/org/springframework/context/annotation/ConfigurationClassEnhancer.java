...
class ConfigurationClassEnhancer {
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		// 增强父类，CGLIB底层实现就是增强父类
		enhancer.setSuperclass(configSuperClass);
		// 增强一个接口
		// EnhancedConfiguration 能够获取BeanFactory
		 ...
		 * 主要为CGLIB生成的类添加成员变量$$BeanFactory
		 * 同时实现了EnhancedConfiguration接口就能获取到BeanFactory
		 * 有了BeanFactory就可以去获得对象，在this调用时拦截获取BeanFactory中的目标bean
		 */
